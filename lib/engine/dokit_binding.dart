...
class DoKitWidgetsFlutterBinding extends WidgetsFlutterBinding with DoKitServicesBinding {
...
class DoKitBinaryMessenger extends BinaryMessenger {
  ...
  ChannelInfo? saveMessage(String name, ByteData? data, bool send) {
    final MethodChannelKit? kit = ApmKitManager.instance.getKit<MethodChannelKit>(ApmKitName.KIT_CHANNEL);
    if (kit == null) {
      return null;
    }
    ...
    try {
      ...
      if (info == null) {
        ...
        info = ChannelInfo(name, call.method, call.arguments, send ? ChannelInfo.TYPE_USER_SEND : ChannelInfo.TYPE_USER_RECEIVE);
        info.methodCodec = codec;
      }
    } catch (e) {
      info = ChannelInfo.error(name, send ? ChannelInfo.TYPE_USER_SEND : ChannelInfo.TYPE_USER_RECEIVE);
    }
    kit.save(info);
    return info;
  }
  ...
  IInfo? filterSystemChannel(String name, ByteData? data, bool send) {
    ...
    if (name == SystemChannels.accessibility.name) {
      return decodeMessage(name, data, SystemChannels.accessibility.codec, send);
    }
    ...
    if (name == SystemChannels.platform_views.name) {
      return decodeMethod(name, data, SystemChannels.platform_views.codec, send);
    }
  ...
  IInfo? decodeByteMessage(String name, ByteData? data, bool send) {
    ...
    return ChannelInfo(name, null, arguments, send ? ChannelInfo.TYPE_SYSTEM_SEND : ChannelInfo.TYPE_SYSTEM_RECEIVE);
  }

  IInfo decodeMessage(String name, ByteData? data, MessageCodec<dynamic> codec, bool send) {
    final dynamic call = codec.decodeMessage(data);
    return ChannelInfo(name, call.toString(), null, send ? ChannelInfo.TYPE_SYSTEM_SEND : ChannelInfo.TYPE_SYSTEM_RECEIVE)
      ..messageCodec = codec;
  }

  IInfo decodeMethod(String name, ByteData? data, MethodCodec codec, bool send) {
    final MethodCall call = codec.decodeMethodCall(data);
    return ChannelInfo(name, call.method, call.arguments, send ? ChannelInfo.TYPE_SYSTEM_SEND : ChannelInfo.TYPE_SYSTEM_RECEIVE)
      ..methodCodec = codec;
  }
}
