...
class DoKitHttpClientResponse implements HttpClientResponse {
  ...
  @override
  HttpClientResponseCompressionState get compressionState => origin.compressionState;
  ...
  StreamSubscription<List<int>> listen(void Function(List<int> event)? onData,
    ...
    if (!isTextResponse()) {
      ...
      return origin.listen(onData, onError: onError, onDone: onDone, cancelOnError: cancelOnError);
    }
    void onDataWrapper(List<int> result) {
      ...
      try {
        ...
        if (encoding != null) {
          recordResponse(statusCode, encoding.decode(result), headers.toString(), contentLength);
        } else {
    ...
    return origin.listen(onDataWrapper, onError: onError, onDone: onDone, cancelOnError: cancelOnError);
  }
  ...
  Stream<S> transform<S>(StreamTransformer<List<int>, S> streamTransformer) {
    ...
    s.listen((S event) {
      if (event is Uint8List) {
        ...
        if (encoding != null) {
          recordResponse(statusCode, encoding.decode(result.toList()), headers.toString(), event.length);
        } else {
          recordResponse(statusCode, '返回结果解析失败', headers.toString(), contentLength);
        }
        ...
        recordResponse(statusCode, 'unknown type:${event.runtimeType}', headers.toString(), contentLength);
      }
    });
    return s;
  }
