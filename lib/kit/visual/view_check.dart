...
class ViewCheckerKit extends VisualKit {
  ...
  String info =
      'Move the center of the screen focus on the control to view the control information';
...
class _ViewCheckerWidgetState extends State<ViewCheckerWidget> {
  ...
  String toInfoString(RenderObjectElement element) {
    ...
    final Offset offset =
        (element.renderObject as RenderBox).localToGlobal(Offset.zero);
    String info = 'widget name: ${element.widget.toString()}\n'
        'widget position: left${offset.dx} top${offset.dy} width${element.size!.width} height${element.size!.height}';
    if (fileLocation != null) {
      info += '\nsource code location: $fileLocation'
